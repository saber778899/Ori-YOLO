...
def get_full_new_annotations(imgs_root, anno_path, save_imgs, debug=False):
    ...
    for imgs_dict in tqdm(imgs_dict_list):
        ...
        for anno_HierRCNN_instance in anno_HierRCNN_list:
            ...
            # 检查 p_bbox 是否包含至少 4 个元素（x, y, w, h），否则跳过
            if len(p_bbox) < 4:
                print(f"[warning] Skipping incomplete bbox for image_id {image_id}: {p_bbox}")
                continue  # 跳过该 bbox，继续处理下一个        
...
if __name__ == '__main__':
    ...
    anno_path_train = os.path.join(dataset_root_path, "annotations_HumanParts/new_data_train_0102.json")
    anno_path_val = os.path.join(dataset_root_path, "annotations_HumanParts/new_data_val_0102.json")
