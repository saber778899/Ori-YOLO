...
def feature_visualization(x, module_type, stage, n=32, save_dir=None):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
    n:              Maximum number of feature maps to plot
    save_dir:       Directory to save results, if None or False, use default 'runs/detect/exp'
    """
    # 处理 save_dir 参数
    if not save_dir:
        save_dir = Path('runs/detect/exp')
    elif isinstance(save_dir, bool):
        save_dir = Path('runs/detect/exp')
    else:
