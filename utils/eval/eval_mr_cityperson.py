...
class COCOMReval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', type='body'):
        '''
        ...
        self.params = Params(iouType=iouType, type=type) # parameters
    ...
    def accumulate(self, p = None):
        ...
        catIds = [1, 2] #_pe.catIds if _pe.useCats else [-1]
        setK = set(catIds)
    ...
    def summarize(self,id_setup):
        ...
        def _summarize(iouThr=None, maxDets=100 ):
            ...
            if len(mrs[mrs<2])==0:
                ...
                #draw_mean_s = np.exp(mean_s)
                #print(list(draw_mean_s))
...
class Params:
    ...
    '''
    def setDetParams(self, type='body'):
        self.imgIds = []
        self.catIds = []
        ...
        #fppi = []
        #while start <= 0:
        #    fppi.append(math.pow(10, start))
        #    start += 0.25
        #print(fppi)
        #self.fppiThrs = np.array(fppi)
        ...
        # elif type == 'face':
        elif type == 'face' or type == 'head':  # add the 'head' type for evaluating body-head matching
            self.HtRng = [[20, 1e5 ** 2], [20, 1e5 ** 2], [20, 1e5 ** 2], [20, 1e5 ** 2]]
        else:
    ...
    def __init__(self, iouType='segm', type='body'):
        if iouType == 'segm' or iouType == 'bbox':
            self.setDetParams(type)
        else:
...
def eval_mr(anno_file, json_result_file, type='body'):
    ...
    for id_setup in range(4):
        ...
        coco_mr_eval = COCOMReval(coco_mod_gt, coco_mod_dt, iou_type, type)
